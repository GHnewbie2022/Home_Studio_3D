...
唯一還沒被測過、且可能對症的設定是「跳過共面鄰體的第一次命中」（非 self-skip）。
...
4. BOX_SKIP_ONLY bed_top 0.347：只補到一半，仍短於 oracle；
   且把 west/east 樑線從 ~0.16 打到 0.085 / 0.106＝傷害（共面排除誤刪合法遮蔽）。
```
...
spike 沒測過：跳過『共面鄰體』的第一次命中（coplanar-neighbor first-hit skip）。

機制：
  bake 射線出發時帶『起點表面所在平面（法線+位置）』與『起點 boxIdx』；
  SceneIntersect（bake-only gated）對候選命中判斷：
    若命中盒為『不同 boxIdx』且其面與起點表面『共面（距離 ≤ ε）』且為『第一次命中』
    → 跳過此命中、射線穿過續打真實環境。
  bounce 與 NEE shadow ray 兩端皆套（光源端 N/A：emitter 命中＝成功收光，見權威表）。
  ray_offset / t_min 留作『同盒自咬』那一截的便宜保險，非主刀。

與 Blender 對應：
  這是 Cycles intersection_skip_self_shadow（兩端 skip）概念，
  從「跳自身 primitive」一般化到「跳共面鄰體 primitive」。屬補課方向、非自創。
```
...
```text
1. 主風險：coplanar 判定的 ε 與「只跳第一次命中」的範圍，能否切得夠準——
   只繞開壞命中、不誤刪合法近接觸遮蔽（即避免 BOX_SKIP 那種把樑打暗的回歸）。
   切得準 → 有機會追平 oracle；切不準 → 重演樑線傷害。這一點只有 B1 施工實測能定。

2. 量級風險：手工補丁靠側向繞開到乾淨牆；coplanar-neighbor skip 靠『穿過鄰體續打環境』。
...
```text
若 B1 施工實測證明 coplanar-neighbor first-hit skip（含 ray_offset/t_min 保險）
...
下一動：
  ...
  2. 使用者拍板是否啟動 B1 施工（建 coplanar-neighbor first-hit skip + 兩端 + 保險，bake-only gated）。
