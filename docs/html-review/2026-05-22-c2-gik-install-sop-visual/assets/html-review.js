(function () {
  ...
  function syncActiveReviewSelection(scrollToMark) {
    ...
    if (!activeId) return;
    const sel = `[data-review-id="${CSS.escape(activeId)}"]`;
  ...
  function anchorDocTop(item) {
    ...
    const sel = `[data-review-id="${CSS.escape(item.id)}"]`;
  ...
  window.addEventListener("resize", () => {
    cancelAnimationFrame(relayoutRaf);
    relayoutRaf = requestAnimationFrame(() => renderSidebar());
  });
  window.addEventListener("load", scheduleCardPositions);
