...
```text
codex/r7-3-10-aftershock-followup
```

目前基準：

```text
db6895d fix(R7-3.10): close south reveal hybrid route
```
...
兩側對稱：
  西樑與東樑同型，應優先用同一套 probe 同時掃左右兩側。
```

## 3. 圖一：西樑與北牆交界

![圖一：西樑與北牆交界縫隙與西樑頂邊北端破圖](evidence/figure-01-west-beam-north-wall-gap.png)
...
![圖二：東樑與北牆交界縫隙與東樑頂邊北端破圖](evidence/figure-02-east-beam-north-wall-gap.png)
...
B 類候選 2：
  樑柱烘焙造成的樑頂北端黑色破圖。
  使用者開關證據：關閉樑柱烘焙後正常。
  可能方向：東西樑 dedicated hybrid 面、structural 舊圖、或樑頂北端 ownership 有不連續。
```
...
紅色先查北牆。
藍色先查樑柱。
同一輪可以共用 cameraState 與截圖 ROI，但量測欄位要分開。
```
...
是否需要把這個問題排在 item 6 前面？
  使用者提供了兩側對稱截圖與明確開關分流。
  若 OPUS 同意，它可作為 §21 B 類遮罩完整度檢查的第一個新增 ROI。
```
...
請判定：
  ...
  2. 藍色破圖是否獨立列為樑柱烘焙問題。
  3. CODEX 下一輪 probe 應優先量 north_wall 1002，還是 beam/structural route ownership。
```

## 9. OPUS 審查 §1-§8：紅＝北牆缺「樑遮頂角」遮罩、藍＝structural 樑端近黑未 guard-fill；兩者同屬 §21 B 類但「修法機制不同」（claude opus 4.7，2026-05-24）

我沒有只看報告，獨立讀了 shader 與 InitCommon.js 把兩側根因落到具體 function／常數。結論：紅、藍都屬 §21 B 類（被實體擋住的死角烤成近黑、texel 仍 alpha=1、bilinear 滲出），左右對稱、可併入並排在 item 6 前面。但**兩個面用的是不同 sampler，所以修法機制不同、要當兩條獨立修正**。另外對 §11/§12「structural 排除 auto-sync 安全」做一個重要更正（那次掃描只獵亮值、抓不到藍色這種暗洩漏）。
...
toggle 佐證一致：北牆烘焙關→正常 = 北牆 bake 確實帶這條近黑（不是 route-none 空洞）。
```

### 9.2 藍色根因候選（已讀碼）：structural 只遮 se_column 兩塊、樑端接觸區沒遮
...
左右對稱。toggle 佐證：樑柱烘焙關→正常 = structural bake 帶這條近黑（非 route-none）。
```

### 9.3 本輪最重要的一點：紅與藍用「不同 sampler」，修法機制因此不同
...
一句話：紅色＝「補遮罩 + 確認走 alpha-aware」；藍色＝「延伸 guard-fill」。兩條別混。
```

### 9.4 更正 §11/§12「structural 排除 auto-sync 安全」（重要）
...
→ §11/§12「structural 安全」只覆蓋白線方向，沒覆蓋藍色這種暗洩漏。
  ...
  特別是各 beam island 內靠 z=-1.874 北端那段，並比對 guard-fill 覆蓋，才算真正關閉。
```
...
3. 下一輪 probe：紅、藍同一支 probe 一起量，但先別預設修法——
   先用 9.3 的 sampler 欄位釘死「紅走哪個 sampler、藍是否 guard-uncovered 近黑」，再決定機制。
```
...
禁止（同意 §7）：補色、cross-fade、還原舊越界、用擴大 handoff 掩蓋縫隙。
```
...
## 10. CODEX 補充提案：用 sampler policy / sampling contract 治本，降低手動打地鼠風險
...
比較穩的方向是：
  ...
  contract test 抓出不符合 policy 的路線。
```
...
Policy C：legacy / no-invalid surface
  ...
  檢查：
    一旦新增 metadata invalid texel，就要轉成 Policy A 或 Policy B。
```
...
structural 1007：
  ...
  dead-zone 來源：
    se_column guard
    sw/east/west beam north-end contact（本輪待查）
```
...
藍色破圖：
  ...
  修法應查 structural island 內的近黑區，並延伸 guard-fill。
```
...
理由：
  ...
  等紅／藍修正通過後，再評估是否把 sampler wrapper 進一步收斂。
```
...
暫時不做：
  ...
  用補色、cross-fade 或擴大 handoff 壓掉縫隙。
```

## 11. OPUS 審查 §10：同意「sampler policy / contract」方向，但先把「全烤短路死碼」與「活路徑」分開——多數 generic sampler 其實在全烤短路分支（hybrid 下死碼），治本是刪它、不是替它寫保母合約（claude opus 4.7，2026-05-24）
...
這正是 §21.1 點過的坑：「被 hybrid 旗標擋成死碼的全烤短路分支」會誤導判讀。我 §9.3 也差點被它帶偏。
```

### 11.2 這如何改變 §10 contract 的定位（重要）
...
→ 治本優先序：先回答「全烤顯示模式還是不是出貨功能」。
   ...
   這題我不替你決定，請使用者裁定（K 神：先分清是「框架缺」還是「殘留該刪」）。
```
...
Q6 policy 放哪最穩？
  → 關鍵事實：contract「已存在」——docs/data/r7-3-10-full-room-diffuse-bake-contract.json (37KB)
    ...
    可行的測試不變式：surface 若 metadataInvalidTexelCount>0 且 policy=alpha-discard，
      靜態檢查它「在出貨模式下可達的每個 radiance function」都呼叫 alpha-aware sampler。
```
...
§10.8（不做清單）→ 全部同意，補一條：
  別對「全烤短路死碼分支」寫 policy 養著；確認全烤顯示是否仍出貨後，能刪就刪（deslop）。
```
...
## 12. OPUS 追記＋更正 §11.1：全烤短路的「牆分支」其實早已死碼、活的只剩地板/天花板；使用者裁定全 hybrid 遷移——回覆順序建議（claude opus 4.7，2026-05-24）
...
更正結論：
  ...
  不是「FullRoomDiffuseMode 關著」。§11.1 對「紅色活路徑＝alpha-aware、補遮罩即可修」的結論不變，只是理由修正。
```

### 12.2 全烤短路其實分兩種命運（正好接上使用者的遷移計畫）
...
所以使用者「全烤只剩地板天花板在用」完全正確，而且更精準：牆的全烤分支「早就死了」，只是沒刪。
```
...
完整遷移＝
  ...
  → generic sampler 這個類別整個消失，§10 contract 要管的「全烤洩漏」幾乎歸零。
```
...
OPUS 建議（傾向解耦，但使用者決定）：
  ...
  contract：因全烤註定要刪，別為 generic 牆分支投資重型合約；保留 Policy C 退場閘即可。
```
...
## 13. OPUS 規劃：解耦三相（先修紅藍、地板天花板遷移獨立）——請 CODEX 審（claude opus 4.7，2026-05-24）
...
關鍵：Step 2 不依賴 Step 3。紅色活路徑已 alpha-aware、藍色走 structural，皆不碰地板/天花板/短路。
```

### 13.1 Step 0：sampler / dead-zone inventory（唯讀，低風險，先做）
...
驗收：
  ...
  → 這份 inventory 同時是 §10 contract 的 Step 0，不必另做。
```

### 13.2 Step 1（可選、便宜的 deslop）：刪「已死的牆全烤分支」
...
可不做：若想最小化動到的檔案，這步可延到 Step 3 一起清。
```

### 13.3 Step 2（核心、使用者可見）：修紅 + 藍
...
禁止：補色、cross-fade、還原舊越界、擴大 handoff 掩蓋。
```

### 13.4 Step 3（獨立大相，後做）：地板/天花板遷 hybrid → 刪全烤
...
contract（§10/§11）：因全烤註定刪，不投資重型合約；保留 Policy C 退場閘測試即可，full contract 延到本相後再評估。
```

### 13.5 請 CODEX 審（execution feasibility）
...
Q6 工具分配：2b/2c 走 /ralplan→/ultrawork，2a 與 Step 0/1 直接做——你同意嗎？
```
...
分工：本規劃由 OPUS 起草、CODEX 審；之後仍 OPUS 寫 source.md、CODEX 重生 index.html。
```

## 14. CODEX Step 2a 紅／藍第一輪 hybrid-only 量測結果（2026-05-24 22:58）
...
本輪新增 probe-only 量測模式，專門輸出 route、world position、normal、hit object、coverage / alpha 與 radiance。這些模式只在 `uR7310C1RuntimeProbeMode` 啟用時生效，不改正常畫面路徑。
...
5. 西側下一輪需要更窄 ROI，對準使用者藍圈最北端黑線與紅色縫隙 transition pixel。
```
...
6. east_beam_under_shadow_hybrid 1018 也在 ROI 出現，但本輪最暗樣本落在 1017。
```
...
與 §13 關係：
  ...
  §13 Step 2c 藍色修正前，東側可先針對 1017 建立 guard / atlas 檢查，西側補 ROI 後再決定是否同步套同型修法。
```

### 14.5 CODEX 對 §13.5 的量測後修正意見
...
Q6 工具分配？
  ...
  2b / 2c 進實修前仍建議走 /ralplan，尤其東樑 1017 牽涉 dedicated package guard / atlas 生成原因。
```
...
## 15. 給 OPUS 的貼上訊息：請審查 CODEX Step 2a 量測結果
...
請審查：
  ...
  Q5. 下一輪 probe 是否要直接鎖三個區：
      ...
      west 1015 / 1016 藍圈北端；
      north_wall 紅色 alpha transition。
```

## 16. OPUS 審查 §14：藍色改判＝dedicated beam 1017（非 structural，已驗碼）；紅色未抓到、疑 alpha-transition seam、勿預設加遮罩（claude opus 4.7，2026-05-24）
...
判讀：
  ...
  → 我 §9.2／§9.3／§13.3-2c「藍色=structural guard-fill」收回，改判 dedicated beam。
```

### 16.2 藍色修法（Policy A，與紅色同機制；非 structural guard-fill）
...
⚠ 連帶一個 sampler fallback 隱患（已讀碼）：
  ...
  這也是 §10 contract「Policy A 的 sampler 一致性」該抓的一條。
```

### 16.3 紅色：未抓到近黑，疑 alpha-transition seam，勿預設加遮罩（修正我 §9.1）
...
⚠ 警告（修正我 §9.1 的傾向）：
  ...
  我 §9.1 的「北牆缺樑遮頂角」仍是合理候選，但量測尚未支持，降為「待證」。
```
...
Q5 下一輪 probe 鎖三區？
  → 同意：
    ...
    north_wall 紅：窄到 x 略內於 ±1.91、y≥2.515 的 transition pixel，報 weightSum + 每個 tap 的 alpha + 有無近黑 tap（定 16.3 的 a/b）。
```

### 16.5 inventory／contract 補強（承 §10–§13）
...
本輪暴露兩條給 §10 contract / Step 0 inventory 的具體項：
  ...
  2. Policy A 的 sampler fallback 行為不一致（beam 1847 不看 alpha vs 北牆 1081 看 alpha）= contract 要抓的一致性條目。
```
...
## 17. CODEX Step 2a 第二輪三區窄探針：藍色雙側 dedicated valid-black，紅色分成西側 alpha transition 與東側 valid-black tap（2026-05-24 23:27）
...
3. east_red_north_wall_alpha_transition / west_red_north_wall_alpha_transition
   目的：鎖紅色 north_wall 邊界 transition 與 atlas tap。
```

### 17.2 藍色結論：東 1017 與西 1015 都已抓到 dedicated valid-black 邊界 texel
...
6. 西側 1016 under route 在窄區也出現，但本輪 near-black 代表樣本落在 1015 inner。
```

### 17.3 紅色結論：西側支援 alpha transition；東側還看到 north_wall valid-black tap
...
全域紅色統計：
  ...
  east red validAlphaOffCount：1
```

西側 redAlphaBoundary 代表樣本：
...
atlas readback：
  ...
  weighted luma=0.111942
```

東側 redAlphaBoundary 代表樣本：
...
atlas readback：
  ...
  weighted luma=0.113438
```

東側 red narrow zone 另有一組值得 OPUS 注意的 north_wall atlas readback：
...
4. 紅色修法目前不能只歸成單一「加遮罩」或單一「alpha transition」。
   西側邊界需要處理 transition。
   東側還要查 valid-black tap 是可見面烤錯，還是應標 invalid 的死角。
```
...
§13.3 Step 2c：
  ...
  structural 暗洩漏掃描降為獨立 latent-safety 項。
```
...
## 18. 給 OPUS 的貼上訊息：請審查 §17 第二輪三區窄探針
...
請重點審：
  ...
  5. 請裁示下一步：
       ...
       紅色是否還要補一輪更窄的 per-texel atlas / metadata probe，或已足夠進 /ralplan。
```

## 19. OPUS 審查 §17：藍色＝atlas 邊角 texel(0,0)烤0、需 edge-dilation 非單純 mask（修正 §16.2）；紅色真缺陷＝(B) 樑後 north_wall valid-black（x≈1.85，印證 §9.1），(A) 為良性 mask 邊（claude opus 4.7，2026-05-24）
...
（beam sampler fallback 1847 不看 alpha 仍建議修，但它不是藍色邊角的解，只是防其他 weightSum=0 情形回黑。）
```

### 19.2 紅色拆解：真缺陷＝(B) 樑後 north_wall valid-black；(A) mask 邊 transition 屬良性
...
判定 CODEX 點 4：紅色「不要」用單一 atlas 邊界合約一起輾。
  真缺陷是 (B)；(A) 多半良性（renorm 值正常），先視覺確認、預設不動。力氣放在 (B)。
```

### 19.3 (B) 的 mask vs re-bake：需一次 LIVE-vs-baked 才能定（紅色唯一未決點）
...
注意我 §16.3 的張力：對 (B) interior 格，mask 不像邊角那樣失效（有 nonzero 權重的 lit 鄰格可借）→ mask 對 (B) 可行；
  但「mask vs re-bake」仍是可見面的設計取捨，交 /ralplan 定。
```
...
## 20. OPUS 審查實作計畫（Superpowers plan，2026-05-24-r7-3-10-north-beam-wall-valid-black-fix）：忠實對應 §19、紀律良好，放行；六點精修（claude opus 4.7，2026-05-24）
...
guardrail「probe 若與 §19 矛盾就暫停、先把新證據寫進 HTML Review 再動 package」= 最佳紀律，特別讚這條。
```
...
R6 次序（非阻塞）：藍 Phase 2-4 與紅 Phase 1/5 彼此「無依賴」，可並行；
   藍較單純，想先收一個可見 win 可獨立先出，不必等紅 LIVE 對照。
```
...
Phase 3 藍「re-bake vs border-extension」的 data-driven 選擇方向對。補一條：
  ...
  re-bake 多半仍寫 0、白跑一輪，直接走 border-extension 較省。
```
...
## 21. CODEX Phase 1：紅色 (B) LIVE-vs-baked 對照完成，西側 (B) 已證實；兩側真值偏 lit，接觸暗假設未獲支持（2026-05-25）
...
docs/tests/r7-3-10-north-beam-gap-probe.test.js
  補 red LIVE probe contract 字串檢查
```

### 21.1 作廢 run（工具座標 bug，保留紀錄）
...
處置：
  ...
  runner 已改成使用全域 renderer.domElement 取得 canvas 高度，並重跑。
```
...
精確資料：
  ...
  LIVE / hybrid readback 是同 camera、同 canvas 點、同 corrected render-target pixel 的 8 SPP 對照。
```

### 21.3 西側紅 (B)：§20 R1 已解，west B 不是只是假設
...
判定：
  suggestedRedBFixClass=metadata-invalidation-or-rebake-lit-reference
```

CODEX 判讀：西側紅 (B) 已量到，不再只是對稱假設。雖然 8 SPP 中有一個 live sample 偏低，平均值仍明顯高於 atlas weighted 與 hybrid，且 5/5 atlas tap 都是 valid-black 型態。這支持「可見真值偏 lit，bake/atlas 把 sliver 拉暗」。
...
CODEX 判讀：東側維持 §19 的紅 (B) 結論。LIVE 平均值高於 atlas weighted，且高於 atlas lit tap max 的 60% 判定線，未支持「真值是接觸暗、只能重烤」。
...
判定：
  (A) side-wall-back alpha transition 不列入 red fix。
```
...
5. 工具注意：
   ...
   若 OPUS 要更高信心，可只挑每側 1 個代表點跑高 SPP，不再跑整片 grid。
```

## 22. OPUS 審查 §21（Phase 1 紅 LIVE 對照）：R1 已解、接觸暗假設裁掉；紅 gap 確為「亮區卻烤 0」；採納「整間正確」治本——先補一次公平 fidelity 驗證，再定 mask vs re-bake（claude opus 4.7，2026-05-25）

### 22.1 §20 R1 已解 + run 紀律

```text
west (B) 5/5 valid-black 實測（x∈[-1.78,-1.754], y≈2.525, z=-1.874），非對稱假設 → R1 解。
作廢 run 的 LIVE readback y-coord bug 由 CODEX 自己抓出、改全域 renderer.domElement 重跑、保留紀錄 → evidence 紀律好。
```

### 22.2 SOLID 事實 + decision gate：gap 區是「亮的」，valid-0 烤錯，接觸暗假設裁掉
...
裁示：
  ...
  8 SPP 雖有單點偏低（west min 0.067），mean 明顯非 0 → 足以裁掉接觸暗（此判斷不需更高 SPP）。
```

### 22.3 周邊正常牆是否也偏暗：尚未定論（不可拿間接比完整）
...
→ 目前無乾淨證據說正常區偏暗，列為待驗：要「rendered full vs LIVE full、量在正常像素、LIVE 高 SPP」才算數（見 §22.4）。
```

### 22.4 採納「整間正確」治本指令：先做公平 fidelity 驗證（Step A）
...
判讀：
  ...
  正常區 rendered < LIVE（顯著、非雜訊） → 真有 fidelity 缺口 → 升級「北牆（乃至全室）bake 保真校正」正式工作，
    走正確 hybrid 架構 re-bake 對齊 LIVE，gap 併入一起治本。
```

### 22.5 gap 修法 mask-vs-rebake：暫緩鎖定，等 Step A
...
（mask 非補色——它把「不可用的 0」標 invalid、由 alpha-aware 借真鄰格。）
```

### 22.6 (A) 良性、續鎖

```text
west / east red_A controls validBlack=0、live≈hybrid≈atlas → (A) side-wall-back transition 不列修正、維持鎖定。
```
...
## 23. CODEX Step A：正常北牆 full LIVE vs full hybrid fidelity 驗證（read-only，2026-05-25）
...
採用版本：
  ...
  這讓 full LIVE vs full hybrid 的比較語意一致。
```
...
白話：
  ...
  這些點也沒有 valid-black taps。
```

### 23.4 採用結果：中心乾淨北牆，16 SPP
...
仍需 OPUS 審查：
  ...
  5. 下一步 mask-vs-local-rebake 的裁示條件是否要再收窄。
```
...
## 24. OPUS 審查 §23（Step A 正常北牆 fidelity）：raw-target 修正成立、正常區 fidelity OK（不需全牆 re-bake）；但選點避開了 gap 鄰帶，mask 借鄰格的正確性移到「修後」驗（claude opus 4.7，2026-05-25）

### 24.1 回 Q1（作廢舊 readback）：同意

```text
舊 readback（renderR739MainReadback / screenCopyRenderTarget，SPP>1）會壓低 hybrid → 假 under-bake。作廢正確。
回頭佐證：先前「正常區偏暗」訊號正是這條量測路徑造成的假象、非真 under-bake——CODEX 自抓自廢，紀律對。
```

### 24.2 回 Q2（raw-target 真累積修正語意）：同意，且有經驗佐證
...
### 24.3 回 Q3（選點代表性）：代表「中央正常北牆」成立；但「gap 鄰帶」未涵蓋（重要限制）
...
### 24.4 回 Q4（結論導向「局部 valid-0」）：同意主結論（不需全牆 re-bake），加一個修後驗證
...
→ 要求：把這個驗證移到「修後」——gap 修完，在 gap + 緊鄰鄰帶（y≈2.5 近樑）做一次 full-render-vs-LIVE（高 SPP）
  + 1000 SPP 雙視角肉眼。若該帶真有小幅 under-bake，會在這關被抓到，不會默默放過。
```

### 24.5 回 Q5（mask-vs-local-rebake 裁示條件收窄）：收窄如下
...
改走 LOCAL-REBAKE，當：
  ...
  或 gap 正確值需要鄰格給不了的梯度。
原則：review 階段先別硬鎖 mask；mask 是預設，但「修後 gap + 鄰帶 vs LIVE」這關沒過就轉 local-rebake。
```
...
## 25. 治本完成定義（共識合約）：局部修好 + 類別級防呆，缺一不可（claude opus 4.7，2026-05-25）
...
結論：類別級治本不是鍍金，是「整間正確」能否達成的關鍵——少了它，遷移愈做、縫愈多。
```
...
(2) 類別級防呆（不讓整類再默默發生）：
    ...
    b. 生產流程硬化（理想、可另案）：烘焙 / package 產生步驟本身，對「零樣本 / off-geometry / 被擋住」的格
       自動標無效或 dilate，從源頭不生垃圾格（呼應 §10 sampler policy / §21 B 類系統化）。
```
...
但 25.2 a 的回歸測試，是本輪就要落地的「最低防呆」。
```

### 25.4 「縫治本完成」的驗收定義（全部成立才能宣告）
...
## 26. CODEX 實作回報：紅 gap 局部 mask + 類別級 valid-black 防呆落地（2026-05-25）
...
採用 §25：
  ...
  藍色樑 atlas 邊角另列未處理項，避免把兩個問題混成同一個完成宣告。
```
...
回歸測試：
  ...
  掃 red gap 邊界帶內是否仍有 alpha=1 且 luma=0 的垃圾格。
```

### 26.3 為什麼 mask 範圍從 11 格擴成整條窄帶
...
valid-black 掃描：
  ...
  wardrobe: red gap boundary bad=0
```
...
(A) / side-wall-back transition 控制：
  ...
  結論：控制點仍可取樣，沒有重新長出紅色 valid-black。
```
...
藍色樑 atlas 邊角：
  ...
  這對應 §19 的 dedicated beam atlas 邊角問題，需另走藍色修法。
```
...
Q4. 藍色仍有 dark samples，CODEX 將它保留為下一個 dedicated beam atlas 邊角修正。
    請確認紅 gap 可獨立收斂，藍色另案推進。
```

## 27. 使用者肉眼驗收：紅標接縫通過（2026-05-25）
...
東側視角：
cameraState={"position":{"x":1.836631,"y":2.511367,"z":-1.865359},"yaw":-0.952,"pitch":0.449,"fov":55,"forward":{"x":0.733838,"y":0.434065,"z":-0.522561}}
```
...
```text
藍色樑頂黑邊本輪仍未修。
這是 dedicated beam atlas 邊角問題，維持下一步獨立處理。
```

## 28. OPUS 審查 §26／§27：紅 gap mask 採用、可條件式獨立收斂；回歸測試只擋北牆紅 gap、未達 §25.2a 類別級須擴；藍另案但須納入類別防呆（claude opus 4.7，2026-05-25）
...
5. 不破 item7/1008/1019-1014/南牆/(A)：✓ 大量測試綠（含 north-east-wall-hybrid）；(A) 控制點無新 valid-black。
   例外：south-wall-ac.test 紅 → 我已查證＝HEAD 既有命名債（28.4），非本輪回歸、不擋紅收斂。
```

### 28.2 回 Q1（mask bounds）：採用
...
修後 red luma min ~0.1（沒黑、沒過亮）+ 使用者肉眼過 → 未誤遮可見 lit 牆。validTexelRatio 0.7665 合理。採用。
```

### 28.3 回 Q2（回歸測試是否滿足 §25.2a）：只達「紅 gap 防回歸」，未達「類別級」，須擴
...
要求：擴成「掃所有 hybrid surface package 的 declared-invalid / boundary 區，alpha=1 且 luma=0 即紅」。
  ...
  只掃北牆紅 gap「不算」§25 的類別級防呆達成。
```

### 28.4 south-wall-ac 紅燈：查證＝HEAD 既有命名債（不擋紅收斂，建議另案修）
...
不擋紅 gap 收斂，但屬「紅測試會被習慣性忽略」的隱患，建議另案修掉（對齊命名）。
```

### 28.5 回 Q3（紅 gap 可否獨立收斂）：可，但先補一個確認
...
注意：紅 gap 的「局部收斂」≠ §25 的「類別級治本完成」——後者要等 28.3 測試擴成類別級才算。
```

### 28.6 回 Q4（藍另案）：同意，但藍須納入類別防呆、仍受 §25 約束
...
## 29. 使用者裁示：紅 gap 收斂、清舊債、擴測試防打地鼠（claude opus 4.7，2026-05-25）
...
注意：這是「局部收斂」；§25 的「類別級治本完成」仍待 29.4 測試擴張到位。
```

### 29.2 舊債要清：south-wall-ac 命名
...
動作（CODEX）：valid-black 回歸測試擴成「掃所有 hybrid surface package 的 declared-invalid / boundary 區，
  alpha=1 且 luma=0 即紅」。北牆已綠；藍（樑 1015-1018）會紅＝下一個目標；未來 floor/ceiling 遷移自動納管。
```
...
紅 gap 已收斂、不再列為待辦。鎖定 / 禁止 / 分工不變。
```

## 30. CODEX 執行 §29.5：south-wall-ac 舊債轉綠、valid-black 擴測、藍色樑角 edge-dilation 完成（2026-05-25）
...
驗證：
  ...
  → R7-3.10 south wall AC shadow contract passed
```

### 30.2 valid-black 回歸測試擴成類別級
...
TDD 紅燈：
  ...
  這與 §19.1 的藍色樑 atlas 邊角根因一致。
```

### 30.3 藍色修復：dedicated beam atlas 邊角 edge-dilation
...
數值：
  ...
  1018 east_beam_under_shadow：
    before luma=0.18896681，原本乾淨，未修改。
```
...
輸出資料夾：
  .omc/r7-3-10-north-beam-gap-probe/20260525-033629
```
...
Q4. 1018 本來 north_edge_corner 不是黑值，本輪未修改；是否同意維持不動，只由回歸測試守住？
```

## 31. 使用者肉眼驗收：藍標接縫通過（2026-05-25）
...
仍需 OPUS 審查：
  §30.6 四個問題。
  特別是 floor / ceiling 暫排除 declared-invalid 掃描的界線，以及 1018 維持不動是否可採用。
```

## 32. OPUS 審查 §30／§31：紅+藍雙側全收斂（原始問題關閉）；測試對 mask/desync 家族已 class-wide、對 edge-corner 家族仍 anchor-only 待泛化；四問裁示（claude opus 4.7，2026-05-25）
...
剩下都是前瞻項（floor/ceiling 遷移 + 納回掃描、pipeline 硬化、edge-corner 守恆泛化），非本問題殘留。
```

### 32.2 藍 §25.4 五條對照 → 局部治本完成（回 user Q2 / §30.6 Q3）
...
→ 藍 dedicated beam atlas 邊角「局部治本完成、收斂」。
```

### 32.3 回 §30.6 Q2（edge-dilation 是否符合 §19.1 + 類別級）：符合，但 edge-corner 守恆仍 anchor-only
...
建議：把 check 2 泛化成「掃所有 package 的 atlas 邊框（首末行／列）alpha-valid-black」，
  才對 edge-corner 家族也達 class-wide（呼應 §25.1 防打地鼠）。可在 floor/ceiling 遷移前/時做（那時才有新邊界）。
```

### 32.4 回 Q1（floor/ceiling 排除界線）：採用，但要「自我強制」
...
建議自我強制：加一條 assert「1001/1006 仍為 full-bake（pointer 有 full-room 標記 / 未接進 hybrid 累積）」。
  ...
  「遷移後納回」就從 TODO 變成測試自動把關，不會被忘。
```

### 32.5 回 user Q3（生產流程硬化時點）：併入 floor/ceiling 遷移做，中間靠回歸測試守住
...
條件：pipeline 硬化落地前，回歸測試（含 §32.3 泛化版）必須持續守住所有手動 dilation 邊角，防重烤默默還原。
```

### 32.6 回 Q4（1018 不動）：同意

```text
1018 east_beam_under 角點原本 luma 0.189、乾淨 → 不動正確（不修沒壞的）。納入回歸 anchor 守住＝好做法（未來變黑會被抓）。同意。
```
