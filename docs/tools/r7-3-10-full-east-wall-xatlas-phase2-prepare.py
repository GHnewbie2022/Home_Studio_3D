#!/usr/bin/env python3
"""R7-3.10 Phase 2: prepare full east-wall XATLAS bake inputs.

東牆版（複製自 north 版，x<->z 角色對調）：
  固定軸 = worldX = 1.91（room-inward 法線 = -X）
  自由軸 v（atlas 列 / height）= worldZ（深度，跨度 4.93m）
  自由軸 u（atlas 行 / width）= worldY（高度，跨度 2.905m）
北牆 byte-identical 管線零改動：此檔為獨立新檔，不碰 north prepare。
"""
...
DEFAULT_OUT_ROOT = REPO / "assets/runtime/r7-3-10/work/r7-3-10-full-east-wall-xatlas-phase2"
PHASE = "r7-3-10-phase2-full-east-wall-xatlas-c1"
...
# 東牆無 A1 木門 ROI 概念；保留欄位、設為整面（密度報告用、非阻塞）。
A1_ROI = {
    "x": 1.91,
    "yMin": 0.0,
    "yMax": 2.905,
...
def build_mesh() -> dict[str, Any]:
    ...
    # v0,v1 共用 yMin（u-min），v0,v3 共用 zMin（v-min）——與 north 版 (xMin/yMin) 結構同型。
    positions = [
        [x, y_min, z_min],
        [x, y_min, z_max],
        [x, y_max, z_max],
...
def build_normal_len_audit(width: int, height: int, formula: dict[str, Any], tile_size: int) -> dict[str, Any]:
    ...
    return {
        "schema": "r7-3-10-full-east-wall-xatlas-normal-len-audit-v1",
...
def build_report(
    ...
    return {
        "schema": "r7-3-10-full-east-wall-xatlas-phase2-prepare-v1",
        "result": result,
        "failures": failures,
        "phase": PHASE,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare R7-3.10 full east-wall XATLAS Phase 2 bake inputs.")
...
def main() -> int:
    ...
    out_dir = args.out_dir if args.out_dir else DEFAULT_OUT_ROOT / timestamp() / "xatlas-bake-full-east-wall"
    if not out_dir.is_absolute():
        out_dir = REPO / out_dir
    out_dir.mkdir(parents=True, exist_ok=True)

    input_path = out_dir / "full-east-wall-xatlas-input-mesh.json"
    uv_path = out_dir / "full-east-wall-xatlas-dry-run-uv.json"
