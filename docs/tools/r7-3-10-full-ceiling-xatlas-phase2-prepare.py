#!/usr/bin/env python3
"""R7-3.10 Phase 2: prepare full ceiling XATLAS bake inputs.

天花板版（依東牆版結構改造）：
  固定軸 = worldY = 2.905（天花板室內側面）
  room-inward 法線 = (0, -1, 0)（朝下、朝室內）
  自由軸 u（atlas 寬 / 行 / UV.x）= worldX（跨度 4.22m）
  自由軸 v（atlas 高 / 列 / UV.y）= worldZ（跨度 5.33m）
東牆 byte-identical 管線零改動：此檔為獨立新檔，不碰 east/north prepare。
"""
...
EXPECTED_ATLAS = {"width": 4265, "height": 3377}
TILE_SIZE = 512
WORLD_BOUNDS = {
    "y": 2.905,
    "xMin": -2.11,
...
# 天花板無門 ROI 概念；設為整面（與 WORLD_BOUNDS 同範圍，密度報告用、非阻塞）。
A1_ROI = {
    "y": 2.905,
    "xMin": -2.11,
    "xMax": 2.11,
...
def build_mesh() -> dict[str, Any]:
    ...
    # 在 y=2.905 平面排 4 角點，使輸出 normal 為 (0,-1,0)（朝下）。
    # winding：v0=[xMin,y,zMin], v1=[xMax,y,zMin], v2=[xMax,y,zMax], v3=[xMin,y,zMax]
    # 三角形 0 邊向量：e01=(+,0,0), e02=(0,0,+), cross=(0,-1,0) ✓
    # v0,v1 共用 xMin 或 xMax？—— v0/v3 共用 xMin（u-min），v0/v1 共用 zMin（v-min）
    #   故 u 軸（worldX）在 v0/v3 取 xMin，v1/v2 取 xMax。
    #   v 軸（worldZ）在 v0/v1 取 zMin，v2/v3 取 zMax。
    positions = [
        [x_min, y, z_min],  # v0：u-min, v-min
        [x_max, y, z_min],  # v1：u-max, v-min
        [x_max, y, z_max],  # v2：u-max, v-max
        [x_min, y, z_max],  # v3：u-min, v-max
    ]
    indices = [
        [0, 1, 2],
        [0, 2, 3],
...
def uv_triangle_record(
    ...
    # 世界面積用兩自由軸 (x,z)：positions[*][0]=x, positions[*][2]=z。
    return {
        "triangleId": triangle_id,
        ...
        "worldAreaAbs": abs(triangle_area_2d([[positions[index][0], positions[index][2]] for index in source_indices])),
    }
...
def uv_formula(mesh: dict[str, Any], uv_output: dict[str, Any]) -> dict[str, Any]:
    ...
    # u 軸 = worldZ：v0,v1 在 zMin（u-min），v2,v3 在 zMax（u-max）。
    u_z_min = (v0[0] + v1[0]) * 0.5
    u_z_max = (v2[0] + v3[0]) * 0.5
    # v 軸 = worldX：v0,v3 在 xMin（v-min），v1,v2 在 xMax（v-max）。
    ...
    return {
        ...
        "orientation": "u increases with worldZ (atlas width); v increases with worldX (atlas height); runtime V is row-flipped",
    }
...
def build_normal_len_audit(width: int, height: int, formula: dict[str, Any], tile_size: int) -> dict[str, Any]:
    # rows（height）= v 軸 = worldX；cols（width）= u 軸 = worldZ。
    x_vals = (np.arange(height, dtype=np.float64) + 0.5) / height
    z_vals = (np.arange(width, dtype=np.float64) + 0.5) / width
    ...
    z01 = (z_vals - u_z_min) / (u_z_max - u_z_min)
    ...
    return {
        "schema": "r7-3-10-full-ceiling-xatlas-normal-len-audit-v1",
...
def write_memmaps(out_dir: Path, width: int, height: int, formula: dict[str, Any], chunk_rows: int) -> dict[str, Any]:
    ...
    z_world_cols = lerp(WORLD_BOUNDS["zMin"], WORLD_BOUNDS["zMax"], z01_cols).astype(np.float32)
    ...
    world_y_fixed = np.float32(WORLD_BOUNDS["y"])
...
def build_report(
    ...
    return {
        "schema": "r7-3-10-full-ceiling-xatlas-phase2-prepare-v1",
        "result": result,
        "failures": failures,
        "phase": PHASE,
        ...
        "unwrap": {
            "nearPlanar": all(abs(point[1] - WORLD_BOUNDS["y"]) <= 1e-6 for point in mesh["positions"]),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare R7-3.10 full ceiling XATLAS Phase 2 bake inputs.")
...
def main() -> int:
    ...
    out_dir = args.out_dir if args.out_dir else DEFAULT_OUT_ROOT / timestamp() / "xatlas-bake-full-ceiling"
    if not out_dir.is_absolute():
        out_dir = REPO / out_dir
    out_dir.mkdir(parents=True, exist_ok=True)

    input_path = out_dir / "full-ceiling-xatlas-input-mesh.json"
    uv_path = out_dir / "full-ceiling-xatlas-dry-run-uv.json"
