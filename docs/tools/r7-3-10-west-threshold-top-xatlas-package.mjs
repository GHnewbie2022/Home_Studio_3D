#!/usr/bin/env node
/*
 * Runtime pointer helper for the west_threshold_top XATLAS bake.
 */
...
const IDENTITY = {
	...
	configId: 1,
	atlasGroup: 'shell',
	masterRectKey: 'west_threshold_top',
	normal: [0, 1, 0],
	worldBounds: { x: [-2.11, -1.91], y: 0.09, z: [-1.874, -0.984] },
};
const ATLAS_W = 160;
const ATLAS_H = 712;
...
function pointerFor(dir, kind, prepareDir) {
	...
	if (w !== ATLAS_W || h !== ATLAS_H) throw new Error(`west_threshold_top atlas size ${w}x${h} != ${ATLAS_W}x${ATLAS_H}`);
	...
	return {
		...
		runtimeScope: 'c1_xatlas_west_threshold_top_runtime',
		runtimeTexture: 'tR7310C1XatlasRuntimeAtlasTexture',
		runtimeArchitecture: 'master_subrect_west_threshold_top',
		...
		phase2: {
			...
			westThresholdTopFullRadianceBake: directLightAlreadyIncluded,
...
function main() {
	...
	const summary = { tool: 'r7-3-10-west-threshold-top-xatlas-package', rawDir: rel(args.rawDir), prepareDir: rel(args.prepareDir), writes: [] };
