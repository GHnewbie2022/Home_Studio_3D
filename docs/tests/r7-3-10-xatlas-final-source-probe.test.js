...
const xatlasRuntimeApplyEnd = shader.indexOf('r7310C1FloorHybridRadiance(hitType, hitObjectID, nl, x)', xatlasRuntimeApply + 1);
...
assert.ok(xatlasRuntimeApplyEnd > xatlasRuntimeApply, 'xatlas runtime first-hit block must be locatable');
const xatlasRuntimeApplyBody = shader.slice(xatlasRuntimeApply, xatlasRuntimeApplyEnd);
assert.match(xatlasRuntimeApplyBody, /uR7310C1XatlasRuntimeCentralDeskDirectIncluded[\s\S]{0,120}r7310XatlasRuntimeCentralDeskFirstHit[\s\S]{0,80}\bbreak\s*;/, 'central desk full-radiance first hit must stop before live direct light');
assert.match(xatlasRuntimeApplyBody, /Indirect XATLAS packages continue to the shared direct-light path/, 'indirect xatlas packages must still document the live direct-light continuation');
