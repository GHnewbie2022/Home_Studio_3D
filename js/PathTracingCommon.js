...
void main( void )
{
	...
	// R6 LGG-r19 J3：借光 pass 進入 accumulator 前對單樣本 clamp（source-side firefly suppression）
	// 不在 source clamp 的話，單一 firefly sample 之值會永久進入 sum，後續 N frame 平均仍殘留可見光斑
	// clamp 1.0 仍涵蓋正常室內 ambient（typically 0.1~0.6 量級），高於此即視為 firefly 截掉
	if (uIsBorrowPass > 0.5)
	{
		currentPixel.rgb = min(currentPixel.rgb, vec3(1.0));
	}
